...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === 'extract_clean_markdown') {
        const url = args?.url;
        const paymentTxHash = args?.paymentTxHash?.trim();
        if (!url) {
            return {
                isError: true,
        ...
        try {
            ...
            const validTransfers = transferLogs.filter((log) => log.address.toLowerCase() === CONFIG.USDC_CONTRACT_ADDRESS.toLowerCase() &&
