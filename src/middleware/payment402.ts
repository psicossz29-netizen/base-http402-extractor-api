...
import { createPublicClient, http, fallback, parseAbiItem, parseEventLogs, type Address } from 'viem';
...
export function createBasePublicClient(rpcUrls: string[] = DEFAULT_BASE_RPCS) {
...
export function paymentRequiredMiddleware(options: PaymentOptions = {}) {
  const priceUsdc = options.priceUsdc ?? CONFIG.SERVICE_PRICE_USDC;
  const recipient = (options.recipient ?? CONFIG.AGENT_PUBLIC_ADDRESS) as Address;
