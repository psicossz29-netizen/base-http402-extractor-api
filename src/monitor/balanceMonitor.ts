...
export interface ProgressState {
  ...
  grossRevenueRecorded: number;
  status: 'IN_PROGRESS' | 'TARGET_REACHED';
  agentPublicAddress: string;
  network: string;
  chainId: number;
  usdcContract: string;
  lastCheckedAt: string;
  remainingUsdcToTarget: number;
...
function loadMilestones(): MilestoneState {
...
async function queryBalanceWithRetry(
...
export async function checkBalanceAndProgress(): Promise<ProgressState> {
  const publicAddress = CONFIG.AGENT_PUBLIC_ADDRESS as Address;
  const usdcAddress = CONFIG.USDC_CONTRACT_ADDRESS as Address;
